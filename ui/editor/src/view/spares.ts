...
function sparesVNodes(
  ctrl: EditorCtrl,
  spares: (Role | 'skip' | SpecialSelected)[],
  color: Color,
): VNode[] {
  ...
  return spares.map((s: Role | 'skip' | SpecialSelected) => {
    ...
    const selectableNode =
      typeof sel === 'string'
        ? h('div', { attrs, props: { innerHTML: svgIcons(sel) } })
        : h('piece', { attrs });
  ...
  return (e: MouchEvent): void => {
    ...
    if (prevSelected && compareSelected(prevSelected, s)) {
      ...
      (!pos ||
        ...
        (Math.abs(pos[0] - ctrl.initTouchMovePos[0]) < 20 &&
          Math.abs(pos[1] - ctrl.initTouchMovePos[1]) < 20))
    ) {
      ctrl.selected(s);
      ctrl.shogiground.state.selectable.deleteOnTouch = s === 'trash';

...
object student {
  ...
  def show(
    ...
    bits.layout(s.user.username, Left(clas withStudents students), s.student.some)(
      ...
      div(cls := "box__pad")(
        ...
        s.student.notes.nonEmpty option div(cls := "student-show__notes")(
          richText(s.student.notes),
        ),
        s.student.managed option div(cls := "student-show__managed")(
          ...
          div(cls := "student-show__managed__actions")(
            postForm(
              action := routes.Clas.studentSetKid(clas.id.value, s.user.username, !s.user.kid),
            )(
              form3.submit(
                if (s.user.kid) trans.disableKidMode() else trans.enableKidMode(),
                icon = none,
              )(
                s.student.isArchived option disabled,
                cls := List(
                  "confirm button button-empty" -> true,
      ...
      created map { case Student.WithPassword(student, password) =>
        div(cls := "student-add__created")(
          strong(
            ...
            pre(
              trans.clas
                .studentCredentials(student.realName, usernameOrId(student.userId), password.value),
            ),
          ),
        )
      },
        ...
        div(cls := "student-add__choice")(
          ...
          postForm(cls := "form3", action := routes.Clas.studentInvite(c.id.value))(
            form3.group(invite("username"), trans.clas.lishogiUsername())(
              form3.input(_, klass = "user-autocomplete")(created.isEmpty option autofocus)(
                dataTag := "span",
              ),
            ),
        ...
        div(cls := "student-add__choice")(
          ...
          postForm(cls := "form3", action := routes.Clas.studentCreate(c.id.value))(
            form3.group(
              ...
              help =
                a(cls := "name-regen", href := s"${routes.Clas.studentForm(c.id.value)}?gen=1")(
  ...
  def edit(clas: Clas, students: List[Student], s: Student.WithUser, form: Form[_])(implicit
      ctx: Context,
  ) =
    bits.layout(s.user.username, Left(clas withStudents students), s.student.some)(
  ...
  def release(clas: Clas, students: List[Student], s: Student.WithUser, form: Form[_])(implicit
    ...
    bits.layout(s.user.username, Left(clas withStudents students), s.student.some)(
      ...
      div(cls := "box__pad")(
        ...
        postForm(
          cls    := "form3",
          action := routes.Clas.studentReleasePost(clas.id.value, s.user.username),
        )(
          form3.globalError(form),
          form3.group(
            form("email"),

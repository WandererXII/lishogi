...
final class Tv(
  ...
  def index = onChannel(lishogi.tv.Tv.Channel.Best.key)

  def onChannel(chanKey: String) = {
    Open { implicit ctx =>
      (lishogi.tv.Tv.Channel.byKey get chanKey).fold(notFound)(lishogiTv)
    }
  }

  def sides(gameId: String, color: String) =
    Open { implicit ctx =>
      OptionFuResult(shogi.Color(color) ?? { env.round.proxyRepo.pov(gameId, _) }) { pov =>
  ...
  def channels = {
    apiC.ApiRequest { _ =>
      ...
      implicit val championWrites = Json.writes[lishogi.tv.Tv.Champion]
  ...
  private def lishogiTv(channel: lishogi.tv.Tv.Channel)(implicit ctx: Context) = {
    OptionFuResult(env.tv.tv getGameAndHistory channel) { case (game, history) =>
      ...
      val onTv = lishogi.round.OnLishogiTv(channel.key, flip)
      negotiate(
        html = env.tournament.api.gameView.watcher(pov.game) flatMap { tour =>
          env.api.roundApi.watcher(pov, tour, lishogi.api.Mobile.Api.currentVersion, tv = onTv.some) zip
  ...
  def games = gamesChannel(lishogi.tv.Tv.Channel.Best.key)

  def gamesChannel(chanKey: String) =
    Open { implicit ctx =>
      (lishogi.tv.Tv.Channel.byKey get chanKey) ?? { channel =>
        env.tv.tv.getChampions zip env.tv.tv.getGames(channel, 15) map {
          case (champs, games) => {
            NoCache {
              Ok(html.tv.games(channel, games map lishogi.game.Pov.first, champs))
            }
          }
        }
      }
  ...
  def feed =
    Action.async {
      ...
      import lishogi.round.TvBroadcast

          ...
          .map {
            case (filterKey, valueKeys) => {
              ...
              filterKey match {
                ...
                case SentePlayStyle.key   => build(SentePlayStyle)
                case GotePlayStyle.key    => build(GotePlayStyle)
                case BishopTrade.key      => build(BishopTrade)
                case RookTrade.key        => build(RookTrade)

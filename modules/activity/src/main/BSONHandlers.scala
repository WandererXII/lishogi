...
private object BSONHandlers {
  ...
  def byUser(userId: User.ID): Bdoc = $doc(
    ActivityFields.userId -> userId,
  )
  ...
  implicit private[activity] lazy val scoreHandler: lila.db.BSON[Score] = new lila.db.BSON[Score] {
    private val win  = "w"
    private val loss = "l"
    private val draw = "d"
    private val rp   = "r"
  ...
  implicit private[activity] lazy val gamesHandler: BSONHandler[Games] =
    typedMapHandler[PerfType, Score](perfTypeKeyIso)
      .as[Games](Games.apply, _.value)

  implicit private[activity] lazy val gameIdHandler: BSONHandler[GameId] =
    BSONStringHandler.as[GameId](GameId.apply, _.value)

  implicit private[activity] lazy val postIdHandler: BSONHandler[PostId] =
    BSONStringHandler.as[PostId](PostId.apply, _.value)
  implicit private[activity] lazy val postsHandler: BSONHandler[Posts] =
    isoHandler[Posts, List[PostId]]((p: Posts) => p.value, Posts.apply _)

  implicit private[activity] lazy val puzzlesHandler: BSONHandler[Puzzles] =
    isoHandler[Puzzles, Score]((p: Puzzles) => p.score, Puzzles.apply _)

  implicit private[activity] lazy val stormHandler: lila.db.BSON[Storm] = new lila.db.BSON[Storm] {
  ...
  implicit private[activity] lazy val corresHandler: BSONDocumentHandler[Corres] =
    Macros.handler[Corres]
  implicit private[activity] lazy val patronHandler: BSONHandler[Patron] =
    BSONIntegerHandler.as[Patron](Patron.apply, _.months)

  implicit private[activity] lazy val followListHandler: BSONDocumentHandler[FollowList] =
    Macros.handler[FollowList]

  implicit private[activity] lazy val followsHandler: lila.db.BSON[Follows] =
  ...
  implicit private[activity] lazy val studiesHandler: BSONHandler[Studies] =
    isoHandler[Studies, List[Study.Id]]((s: Studies) => s.value, Studies.apply _)
  implicit private[activity] lazy val teamsHandler: BSONHandler[Teams] =
  ...
  implicit lazy val activityHandler: lila.db.BSON[Activity] = new lila.db.BSON[Activity] {
    ...
    def writes(w: lila.db.BSON.Writer, o: Activity) =
      BSONDocument(
        ...
        expiresAt -> o.day.toDate.plusDays(Activity.expireAfterDays),
      )
  }
}

...
final class PlanApi(
            ...
            userPatron(user).flatMap {
              case None =>
                patronColl.insert.one(
                  ...
                  setDbUserPlan(user, lishogi.user.Plan.start) >>
  ...
  private def setDbUserPlanOnCharge(user: User, patron: Patron): Funit = {
    ...
    Bus.publish(lishogi.hub.actorApi.plan.MonthInc(user.id, plan.months), "plan")
  ...
  def recentChargesOf(user: User): Fu[List[Charge]] =
    chargeColl.ext.find($doc("userId" -> user.id)).sort($doc("date" -> -1)).cursor[Charge]().list()
  ...
  private def addCharge(charge: Charge): Funit =
    chargeColl.insert.one(charge).void >>- {
      ...
      monthlyGoalApi.get foreach { m =>
        Bus.publish(
          lishogi.hub.actorApi.plan.ChargeEvent(
        ...
        lishogi.mon.plan.goal.update(m.goal.value)
        lishogi.mon.plan.current.update(m.current.value)
        lishogi.mon.plan.percent.update(m.percent)
        if (charge.isPayPal) lishogi.mon.plan.paypal.record(charge.cents.value)
        else if (charge.isStripe) lishogi.mon.plan.stripe.record(charge.cents.value)
      }
    }
  ...
  private def setDbUserPlan(user: User, plan: lishogi.user.Plan): Funit =

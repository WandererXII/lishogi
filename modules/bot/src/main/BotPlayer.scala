...
import lishogi.hub.actorApi.round.{ Abort, BotPlay, RematchNo, RematchYes, Resign }
import lishogi.round.actorApi.round.{ DrawNo, DrawYes }
...
final class BotPlayer(
  ...
  private def clientError[A](msg: String): Fu[A] = fufail(lishogi.round.ClientError(msg))

  def apply(pov: Pov, me: User, uciStr: String, offeringDraw: Option[Boolean]): Funit =
    lishogi.common.Future.delay((pov.game.hasAi ?? 500) millis) {
      Uci(uciStr).fold(clientError[Unit](s"Invalid UCI: $uciStr")) { uci =>
        lishogi.mon.bot.moves(me.username).increment()
  ...
  def chat(gameId: Game.ID, me: User, d: BotForm.ChatData) =
    fuccess {
      lishogi.mon.bot.chats(me.username).increment()
      val chatId = lishogi.chat.Chat.Id {
        if (d.room == "player") gameId else s"$gameId/w"
      }
      val source = d.room == "spectator" option {
        lishogi.hub.actorApi.shutup.PublicSource.Watcher(gameId)
      }
  ...
  private def rematch(id: Game.ID, me: User, accept: Boolean): Fu[Boolean] =
    gameRepo game id map {
      _.flatMap(Pov(_, me)).filter(p => isOfferingRematch(!p)) ?? { pov =>
        ...
        lishogi.common.Future.delay(if (accept) 100.millis else 2.seconds) {
          fuccess {
